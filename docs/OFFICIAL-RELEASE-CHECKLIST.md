...
prepared dean/staff review emails to omit a phone number and authenticator MFA.
The authenticated account must also have a staff role. All other accounts
remain phone- and MFA-mandatory.
...
2. Delete `src/lib/reviewAccess.ts` and the temporary calls to
   `canBypassReviewPhone` / `canBypassReviewMfa` in `src/App.tsx` and
   `src/pages/Auth.tsx`.
